//v1.0  pergésmentesítés 1
//todo: redőny pedál


// debouncing
#define debouncing_samples 2
...
byte keys_last[2][61];
byte keys_old[2][debouncing_samples][61];
byte old_index = 0;
...
void MIDI_read() {
  if (Serial.available() > 2) {   //3 bájt egy midi üzenet
    ...
    if (data1 < 30) {  //csak akkor foglalkozom vele, ha be tudom írni a registers tömbbe, vagyis az elem számnál kisebb
      if (status == noteOn + register_ch) {
        registers[data1] = 1;
      } else if (status == noteOff + register_ch) {
        registers[data1] = 0;
      } else if (status == noteOn + preset_ch && data1 < 7) {
        presets[data1] = 1;
      } else if (status == noteOff + preset_ch && data1 < 7) {
        presets[data1] = 0;
      }
      update_leds();
    }
...
void compare_and_send_debounced(byte cur_array[], byte old_array[][61], int length, int channel, int shift) {
  int send_flag;
  for (int i = 0; i < length; i++) {
    send_flag = 1;
    if (cur_array[i] == 1 && old_array[old_index][i] == 0) {
      for(int j = 0; j < debouncing_samples - 1; j++) {
        if(old_array[(j+old_index)%debouncing_samples][i] == cur_array[i]) {
          send_flag = 0;
        }
      }
      if(send_flag) {
    ...
    } else if (cur_array[i] == 0 && old_array[old_index][i] == 1) {
      for(int j = 0; j < debouncing_samples - 1; j++) {
        if(old_array[(j+old_index)%debouncing_samples][i] == cur_array[i]) {
          send_flag = 0;
        }
      ...
      if(send_flag) {
        MIDI_send(noteOn + channel, i + shift, velocity);  //az invertált logika miatt noteOff kell, ha felfutó él volt
      }
    }
    old_array[(1+old_index)%debouncing_samples][i] = cur_array[i];
  }
  if(old_index == debouncing_samples)
      old_index = 0;
  else
    old_index++;
}
