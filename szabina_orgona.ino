...
byte keys_last[2][61];
byte keys_old[debouncing_samples][2][61];
...
void compare_and_send_debounced(byte cur_array[], byte last_array[], byte old_array[][], int length, int channel, int shift) {
  for (int i = 0; i < length; i++) {
    if (cur_array[i] == 1 && last_array[i] == 0) {
      for(int j = 0; j < debouncing_samples; j++) {
        if(last_array[i])
      }
      MIDI_send(noteOff + channel, i + shift, velocity);  //az invertált logika miatt noteOff kell, ha felfutó él volt
      last_array[i] = cur_array[i];
